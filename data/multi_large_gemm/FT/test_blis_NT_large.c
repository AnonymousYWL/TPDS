...
int main()
{
	...
	FILE *fp;
	if( (fp=fopen("blis_large_NT_FT.txt","w")) == NULL )
	{
	...
	for (j = 0 ; j < 19; j++)
	{
		...
		for(pc = 0; pc < 1; pc++)
		{
			...
			for( i =0 ;i< 1; i++)
			  bli_sgemm(BLIS_NO_TRANSPOSE, BLIS_TRANSPOSE, M,N,K,&alpha,A,K,1,B,K,1,&beta,C,N,1);
		...
		//	random_matrix1(5000,5000,flush);
			...
			printf("\nblis:  M= %d N=%d K=%d flops = %lf effic= %.3lf %\n", 
