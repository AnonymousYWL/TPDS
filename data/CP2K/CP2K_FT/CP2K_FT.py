...
f1 = open("blis_CP2K_FT.txt")
xy1 = f1.read().split("\n")

f2 = open('openblas_CP2K_FT.txt', 'r')
xy2=f2.read().split("\n")

f3 = open("armpl_CP2K_FT.txt")
xy3 = f3.read().split("\n")

f4 = open("libxsmm_CP2K_FT.txt")
xy4 = f4.read().split("\n")

f5 = open("blasfeo_CP2K_FT.txt")
xy5 = f5.read().split("\n")

f6 = open("NSMM_CP2K_FT.txt")
xy6 = f6.read().split("\n")
...
plt.legend((A[0], B[0], C[0],D[0],E[0],F[0]), ('BLIS','ARMPL', 'OpenBLAS', 'BLASFEO','LIBXSMM','LibShalom2'),
		loc="upper right" , borderaxespad=0.,bbox_to_anchor=(0.96,0.98), columnspacing = 0.05,ncol=3,
		frameon=False, prop =font2)
...
f.savefig('CP2K_FT.pdf', bbox_inches='tight')