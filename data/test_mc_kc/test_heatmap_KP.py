...
for item in xy1:
	...
	if(len(arr) != 0 and arr[0]!= ''):
		count = count + 1
		temp_y.append(float(arr[2])/64/41.6 *100)
		if(count % 10 == 0):
			y.append(temp_y)
			temp_y=[]
...
ax=sns.heatmap(y, ax=ax,vmin=69,vmax=84,cmap='YlGnBu_r',fmt='.1f',annot=True,linewidths=2,cbar=True)
...
f.tight_layout()
plt.show()
f.savefig('test_mc_kc_KP.pdf', bbox_inches='tight')